                ...
                for (Object argObject : joinPoint.getArgs()) {
                    ...
                    if (argObject instanceof HttpFrontRequest) {
                        ...
                        if (StringUtil.isBlank(loginToken)) {
                            logger.error("User authorize failed! Login token not existed, url[{}], request[{}]", requestUri, ((HttpFrontRequest) argObject));
                        ...
                        if (StringUtil.isBlank(httpSessionId)) {
                            logger.error("User authorize failed! Session not existed, url[{}], request[{}]", requestUri, ((HttpFrontRequest) argObject));
                        ...
                        if (StringUtil.isBlank(sessionToken)) {
                            logger.error("User authorize failed! Session token not existed, url[{}], request[{}]", requestUri, ((HttpFrontRequest) argObject));
                            throw new RnbRuntimeException("999403", "session.token.not.existed");
                        }
                        if (!sessionToken.equals(loginToken)) {
                            logger.error("User authorize failed! Login token error, url[{}], request[{}]", requestUri, ((HttpFrontRequest) argObject));
